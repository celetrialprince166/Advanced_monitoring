...
pipeline {
    ...
    stages {
        ...
        stage('SonarCloud Analysis') {
            agent { docker { image 'sonarsource/sonar-scanner-cli:5'; reuseNode true } }
            steps {
                    ...
                    sh '''
                        ...
                        for i in $(seq 1 $MAX_RETRIES); do
                            ...
                            if echo "200 301 302" | grep -qw "$HTTP_CODE"; then
                                echo "[PASS] ECS smoke test passed (HTTP $HTTP_CODE)"
                                exit 0
                            fi
                        ...
                        echo "[FAIL] ECS smoke test failed after $MAX_RETRIES attempts"
                        exit 1
                    '''
                }
            }
            ...
            script {
                try {
                    slackSend(
                        ...
                        color: 'danger',
                        tokenCredentialId: 'slack-token',
                        message: "[FAILED] *Build Failed* — Notes App\n*Branch:* `${env.BRANCH_NAME}`\n*Commit:* `${env.GIT_COMMIT_SHORT}` by ${env.GIT_AUTHOR}\n*Message:* ${env.GIT_MESSAGE}\n*Build:* <${env.BUILD_URL}|#${env.BUILD_NUMBER}>"
                    )
                } catch (Exception e) {
                    echo "[WARNING] Slack notification failed: ${e.message}"
                }
            ...
            script {
                try {
                    slackSend(
                        ...
                        color: 'warning',
                        tokenCredentialId: 'slack-token',
                        message: "[UNSTABLE] *Build Unstable* — Notes App\n*Branch:* `${env.BRANCH_NAME}`\n*Build:* <${env.BUILD_URL}|#${env.BUILD_NUMBER}>"
                    )
                } catch (Exception e) {
                    echo "[WARNING] Slack notification failed: ${e.message}"
                }
